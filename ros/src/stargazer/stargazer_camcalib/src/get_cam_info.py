...
# a node that publishes camera info (intrisic parameters) as required by the hand-eye calibration process
# Takes an OpenCV calibration yaml file as input
...
def yaml_to_CameraInfo(yaml_fname):
    ...
    # print(calib_data["camera_matrix"])
    intrinsic_params = [n for row in calib_data["camera_matrix"] for n in row]
    camera_info_msg.K = intrinsic_params
    camera_info_msg.D = calib_data['dist_coefs'][0]

    camera_info_msg.R[0] = 1
    camera_info_msg.R[4] = 1
    camera_info_msg.R[8] = 1

    camera_info_msg.P[0 * 4 + 0] = intrinsic_params[0]
    camera_info_msg.P[0 * 4 + 2] = intrinsic_params[2]
    camera_info_msg.P[1 * 4 + 1] = intrinsic_params[1 * 3 + 1]
    camera_info_msg.P[1 * 4 + 2] = intrinsic_params[1 * 3 + 2]
    camera_info_msg.P[2 * 4 + 2] = 1
